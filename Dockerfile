...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/elixir_hello ./

USER nobody

ENV PHX_SERVER=true

COPY extrametadata.properties .
RUN mkdir myfolder
COPY myfolder/ ./myfolder
COPY hiddenpath.properties .

ENTRYPOINT ["/app/bin/elixir_hello", "start"]